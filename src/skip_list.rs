...
impl SkipListFileHandler {
    ...
    fn read_config_file(&self) -> Res<Vec<usize>>{
        ...
        Ok(rows_bytes.chunks(SIZE_OF_USIZE).map(|x|usize::from_be_bytes(x.try_into().unwrap())).collect())
    }
    ...
    #[inline]
    fn write_pointer_to_bytes_in_offset(&self,offset:usize,len:usize,bytes:&mut [u8],data:&Option<usize>){
        self.write_usize_to_bytes_in_offset(offset,len,bytes,data.unwrap_or(NULL_NODE_ID));
    }
    fn write_usize_to_bytes_in_offset(&self,offset:usize,len:usize,bytes:&mut [u8],data:usize){
    ...
    fn convert_node_to_header(&self, node:&Node)->[u8;NODE_HEADER_SIZE]{
        ...
        //write pointers
        self.write_pointer_to_bytes_in_offset(NEXT_OFFSET,SIZE_OF_NEXT_NODE, &mut ret,&node.next_node);
        self.write_pointer_to_bytes_in_offset(PREV_OFFSET,SIZE_OF_PREV_NODE, &mut ret,&node.prev_node);
        self.write_pointer_to_bytes_in_offset(TOP_OFFSET,SIZE_OF_TOP_NODE, &mut ret,&node.top_node);
        self.write_pointer_to_bytes_in_offset(DOWN_OFFSET,SIZE_OF_DOWN_NODE, &mut ret,&node.down_node);

        //write key and value len
        self.write_usize_to_bytes_in_offset(KEY_SIZE_OFFSET,SIZE_OF_KEY_SIZE,&mut ret,node.key.len());
        self.write_usize_to_bytes_in_offset(VALUE_SIZE_OFFSET,SIZE_OF_VALUE_SIZE,&mut ret,node.value.len()*SIZE_OF_USIZE);
        ret
    }
    ...
    fn new_node(&self,key:String,value:Option<Vec<usize>>) -> Res<Node>{
        ...
        let value=node.value.iter().flat_map(|&x| x.to_be_bytes().to_vec()).collect::<Vec<u8>>();
        let id={
    ...
    fn read_pointer_from_bytes_in_offset(&self,offset:usize,len:usize,bytes:&[u8])->Option<usize>{
    ...
    fn read_node(&self,node_id:usize)->Res<Node>{
        ...
        let next=self.read_pointer_from_bytes_in_offset(NEXT_OFFSET,SIZE_OF_NEXT_NODE,&node_header);
        ...
        let key_len=usize::from_be_bytes(node_header[KEY_SIZE_OFFSET..(KEY_SIZE_OFFSET+SIZE_OF_KEY_SIZE)].try_into().unwrap());
        let mut key_bytes=vec![0;key_len];
        file.read_exact(&mut key_bytes)?;
        let key=String::from_utf8(key_bytes)?;
        let prev=self.read_pointer_from_bytes_in_offset(PREV_OFFSET,SIZE_OF_PREV_NODE,&node_header);
        let top=self.read_pointer_from_bytes_in_offset(TOP_OFFSET,SIZE_OF_TOP_NODE,&node_header);
        let down=self.read_pointer_from_bytes_in_offset(DOWN_OFFSET,SIZE_OF_DOWN_NODE,&node_header);

        Ok(if let NodeType::Data=node_type {
            let value_len = usize::from_be_bytes(node_header[VALUE_SIZE_OFFSET..(VALUE_SIZE_OFFSET + SIZE_OF_VALUE_SIZE)].try_into().unwrap());
            let mut value_bytes = vec![0; value_len];
            file.read_exact(&mut value_bytes)?;
            let value = value_bytes.chunks(SIZE_OF_USIZE).map(|x| usize::from_be_bytes(x.try_into().unwrap())).collect();
            Node{
                id: node_id,
                node_type,
                key,
...
mod pager_tests {
    ...
    fn test_delete_node() {
        ...
        pager.delete_node(&mut node, None).expect("err when deleting node:");
        let read_node = pager.read_node(node.id).unwrap();
    ...
    fn update_node() {
        ...
        pager.update_node_value(&mut node).expect("err when updating node:");
    ...
    fn test_header_of_row_node() {
        ...
        assert_eq!(0, usize::from_be_bytes(header[KEY_SIZE_OFFSET..(KEY_SIZE_OFFSET+SIZE_OF_KEY_SIZE)].try_into().unwrap()))
    }
    #[test]
...
impl SkipList {
    ...
    pub fn insert(&mut self,key:String,value:Vec<usize>) -> Res<()>{
        if let Some(&head_id) = self.rows.last() {
            ...
            if node.key == key { // if node exists add the value
...
mod skiplist_tests {
    ...
    fn complete_test() {
        ...
        let mut skip_list = SkipList::new(&TEST_NAME.to_string()).unwrap();
        for i in 0..range {
            skip_list.insert(i.to_string(), vec![i]).expect(&format!("cant insert {}", i));
            println!("inserted {i}");
        }
        for i in 0..range {
            assert_eq!(skip_list.search(&i.to_string()).expect(&format!("cant lookup {}", i)), Some(vec![i]));
            println!("found {i}");
        }
        for i in 0..range{
            println!("updating {i}");
            skip_list.insert(i.to_string(), vec![i+1]).expect(&format!("cant insert {}", i));
        }
        for i in 0..range {
            let search_res=skip_list.search(&i.to_string()).expect(&format!("cant lookup {}", i));
            println!("found {:?}",search_res);
            assert_eq!(search_res, Some(vec![i,i+1]));
            println!("found updated {i}");
        }
        for i in 0..range {
            skip_list.delete(&i.to_string()).expect(&format!("cant delete {}", i));
            println!("deleted {i}");
        }
        for i in 0..range {
            assert_eq!(skip_list.search(&i.to_string()).expect(&format!("cant lookup {}", i)), None); // check if all deleted
    ...
    fn test_read_existing_list() {
        ...
        let mut skip_list=SkipList::new(&TEST_NAME.to_string()).unwrap();
        for i in 0..range {
            assert_eq!(skip_list.search(&i.to_string()).expect(&format!("cant search {}", i)),Some(vec![i]));
        }
        teardown();
    }
}
