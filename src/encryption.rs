...
pub fn decrypt(key: &String, encrypted: Vec<u8>) -> String {
    String::from_utf8(aes128::decrypt_aes128(&pad_key(key), &encrypted))
        .unwrap()
        .trim_end()
        .to_string()
}
...
impl EncryptionService {
    fn new() -> EncryptionService {
        EncryptionService {
            threadpool_connection: ThreadPool::get_instance(),
        }
    }
    ...
    pub fn get_instance() -> &'static mut RwLock<Self> {
        ...
        unsafe { SINGLETON.as_mut().unwrap() }
    }
}
#[cfg(test)]
mod tests {
    use super::{decrypt, encrypt, pad_key};
