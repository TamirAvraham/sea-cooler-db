...
impl<F> Node<F>{
    ...
    pub fn insert(&mut self,path:&str,handler:F){
        match path.split_once('/') {
            ...
            Some((root,path)) => {
                let node=self.nodes.iter_mut().find(|n|n.key==root || n.is_wild_card);
                match node {
                    None => {
...
mod tests {
    ...
    fn test_get() {
        ...
        assert_eq!(tree.get("/hello/world/hello/world/hello").unwrap(), &"world");
        assert_eq!(tree.get("/hello/world/hello/world/hello/world").unwrap(), &"hello");
        assert_eq!(tree.get("/hello/world/hello/world/hello/world/hello").unwrap(), &"world");
        assert_eq!(tree.get("/hello/world/hello/world/hello/world/hello/world").unwrap(), &"hello");
