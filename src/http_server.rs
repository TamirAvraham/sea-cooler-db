...
impl HttpServer {
    ...
    pub fn add_route(&mut self, method:HttpMethod, path:&str, handler: HttpHandlerFunction){
        ...
        router.entry(method).or_insert(RadixTree::new("/")).insert(path, handler);
    }
    ...
    fn handle_connection(router:&Arc<RwLock<HashMap<HttpMethod, RadixTree<HttpHandlerFunction>>>>,mut stream: TcpStream){
...
impl HttpResponse{
    ...
    pub fn new_internal_error()->HttpResponse {
        Self::new_from_html_file(HttpStatusCode::InternalServerError, "static/500.html".to_string()).unwrap()
    }
...
mod tests {
    ...
    fn test_http_server() {
        ...
        server.add_route(HttpMethod::GET, "/test", |_|{
            Some(HttpResponse::new_from_html_file(HttpStatusCode::OK, "static/test.html".to_string()).unwrap())
        });
    ...
    fn set_up_server() {
        ...
        server.add_route(HttpMethod::GET, "/index", |_|{
            Some(HttpResponse::new_from_html_file(HttpStatusCode::OK, "static/index.html".to_string()).unwrap())
        });
